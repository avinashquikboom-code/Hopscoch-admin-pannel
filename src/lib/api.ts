 ...
 */
export const API_BASE = process.env.NEXT_PUBLIC_API_URL || 'http://192.168.1.103:5001';
...
export const api = {
  ...
  // Orders — admin endpoints (see /api/v1/admin/orders)
  orders: {
    getAll: (params?: Record<string, string>) => {
      const qs = params ? '?' + new URLSearchParams(params).toString() : '';
      return apiRequest<any>(`/api/v1/admin/orders${qs}`);
    },
    getById: (id: string | number) => apiRequest<any>(`/api/v1/admin/orders/${id}`),
    updateStatus: (id: string | number, status: string) =>
      apiRequest<any>(`/api/v1/admin/orders/${id}`, { method: 'PATCH', body: JSON.stringify({ status }) }),
    getTimeline: (id: string | number) => apiRequest<any>(`/api/v1/admin/orders/${id}/timeline`),
    cancel: (id: string | number) =>
      apiRequest<any>(`/api/v1/admin/orders/${id}`, { method: 'PATCH', body: JSON.stringify({ status: 'CANCELLED' }) }),
  },
